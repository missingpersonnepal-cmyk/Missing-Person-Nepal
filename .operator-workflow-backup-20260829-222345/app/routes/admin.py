...
from ..services.candidate_chatgpt_prefill import build_candidate_chatgpt_prefill_prompt
...
from ..services.priority_sources import priority_manual_searches
...
async def discovery_to_submission(request: Request, candidate_id: int):
    ...
    with SessionLocal() as db:
        ...
        notes_parts = []
        if source_post_text:
            notes_parts.append("Public post text:\n" + source_post_text[:12_000])
        elif candidate.snippet:
            notes_parts.append("Search/public snippet:\n" + candidate.snippet)
        if ocr_text:
            notes_parts.append("OCR text from reviewed source image:\n" + ocr_text)
        notes = "\n\n".join(part.strip() for part in notes_parts if part.strip()) or None
