...
def _build_identification_details(snippet: str, source_text: str, ocr_text: str, combined: str) -> str | None:
    details: list[str] = []
    ...
    if snippet.strip():
        add("Search snippet", snippet[:1800])

    if source_text.strip():
        add("Public post text", source_text[:5000])
...
def extract_candidate_prefill(
    ...
    combined = "\n".join(
        part for part in (title, source_text, snippet, ocr_text) if part
    ).strip()
    normalized = _normalize_digits(combined)

    name_ne = _extract_nepali_name(normalized)
    name = _extract_name(title, source_text or snippet, ocr_text)
...
_ORG_WORDS = {
    "news", "update", "hospital", "health", "police", "army", "bank",
    "travel", "nurse", "foundation", "official", "media", "project",
    "hydropower", "school", "college", "nepal", "rasuwa", "flood",
}


def _looks_like_explicit_person_name(value: str) -> bool:
    value = _clean_name(value)
    words = value.split()
    if not (1 <= len(words) <= 5):
        return False
    if any(word.casefold().strip(".'’-") in _ORG_WORDS for word in words):
        return False
    if any(word.casefold() in {"their", "his", "her", "sons", "daughters", "family"} for word in words):
        return False
    for word in words:
        cleaned = word.strip(".'’-")
        if not cleaned:
            return False
        if cleaned[0].isalpha() and not cleaned[0].isupper():
            return False
    return True


def extract_candidate_people_names(
    candidate: DiscoveryCandidate,
    source_text: str = "",
    ocr_text: str = "",
) -> list[str]:
    """Return explicitly written possible person names from one public source.

    The result is only a review aid. It never identifies anyone from a photo.
    """
    title = candidate.title or ""
    snippet = candidate.snippet or ""
    sources = [title, source_text, snippet, ocr_text]
    names: list[str] = []
    seen: set[str] = set()

    def add(value: str) -> None:
        value = _clean_name(value)
        if not _looks_like_explicit_person_name(value):
            return
        folded = value.casefold()
        if folded in seen:
            return
        seen.add(folded)
        names.append(value[:255])

    # Repeated explicit labels, including list-style posts.
    label_pattern = re.compile(
        r"(?:Person\s+Missing|Missing\s+Person(?:\s+Name)?|Name)\s*[:\-]?\s*"
        r"([^\n.;]{2,180})",
        re.IGNORECASE,
    )
    ...
    for source in sources:
        ...
        for match in label_pattern.finditer(source):
            ...
            segment = re.split(
                r"\b(?:has|have|was|is|from|aged?|age|last|works?|working|location|contact)\b",
                segment,
                maxsplit=1,
                flags=re.IGNORECASE,
            )[0]
            for piece in re.split(r"\s+(?:and|&)\s+|[,·•|↪]+", segment):
                piece = piece.strip(" ,:;-–—")
                if piece:
                    add(piece)

    # Capture explicit two-name constructions such as
    # "Dorje Tamang and Pasang Tamang have been out of contact".
    pair_pattern = re.compile(
        r"\b([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){0,3})\s+and\s+"
        r"([A-Z][A-Za-z'.-]+(?:\s+[A-Z][A-Za-z'.-]+){0,3})\s+"
        r"(?:has|have|are|were|is|was|missing|out\s+of\s+contact)\b",
        re.IGNORECASE,
    )
    for source in sources:
        if not source:
    ...
    primary = _extract_name(title, source_text or snippet, ocr_text)
    if primary:
        add(primary)
        primary_folded = primary.casefold()
        names.sort(key=lambda item: 0 if item.casefold() == primary_folded else 1)
...
_MULTI_PERSON_TERMS = (
    ...
    "व्यक्तिहरू",
    "बेपत्ता व्यक्तिहरू",
    "सम्पर्कविहीन व्यक्तिहरू",
)


def candidate_mentions_multiple_people(
    candidate: DiscoveryCandidate,
    source_text: str = "",
    ocr_text: str = "",
) -> bool:
    """Flag explicit public text indicating more than one missing person.

    This is a review hint only. It does not infer identity from any image.
    """
    names = extract_candidate_people_names(
        candidate,
    ...
    return any(term.casefold() in text for term in _MULTI_PERSON_TERMS)
