...
async def discovery_chatgpt_prefill_batch(request: Request, candidate_id: int):
    ...
    with SessionLocal() as db:
        ...
        if candidate.status in {"irrelevant", "rejected"}:
            return HTMLResponse("Restore this candidate before creating submissions.", status_code=409)
        ...
        for person_data in payload["people"]:
            ...
            # Batch mode is deliberately conservative. Exact existing/pending
            # names need the operator's individual attach-vs-new decision.
        ...
        audit(
            db,
            request,
            "batch_chatgpt_prefill_to_submissions",
            "discovery_candidate",
            candidate.id,
            f"created={created}; duplicates={duplicates}; skipped={skipped}",
        )
        db.commit()
