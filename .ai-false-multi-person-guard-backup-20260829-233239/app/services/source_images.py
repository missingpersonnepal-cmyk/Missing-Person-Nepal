...
def extract_public_post_text_from_html(html: str) -> str | None:
    """Extract the richest public post text exposed in HTML metadata."""
    ...
    for key in ("og:description", "twitter:description", "description"):
        ...
        if tag is not None:
            value = _clean_public_post_text(str(tag.get("content") or ""))
            if value:
    ...
    def collect_json(value) -> None:
        if isinstance(value, dict):
            for key, item in value.items():
                if key in {"articleBody", "description", "text", "caption"} and isinstance(item, str):
                    cleaned = _clean_public_post_text(item)
                    if cleaned:
    ...
    for candidate in candidates:
        ...
        if folded in {"facebook", "log into facebook", "log in to facebook"}:
            continue
        if "log into facebook" in folded and len(candidate) < 180:
            continue
        if folded in seen:
            continue
        seen.add(folded)
        filtered.append(candidate[:12_000])
